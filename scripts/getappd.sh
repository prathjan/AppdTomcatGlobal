...
install="$(curl -s --location --request GET "${URL}/zero/v1beta/install/installCommand?sudo=true&multiline=false&application=${APP_NAME}&accessKey=accesskey&serviceUrl=${URL}" \
--header "Authorization: Bearer ${temp}" --data-raw '')"
